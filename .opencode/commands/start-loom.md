...
## Step 2 — Start Loom nREPL server

Start the Loom nREPL server in the background:

```bash
cd /home/denis/Projects/loom && clojure -M:dev -e "(require '[loom.core :as loom]) (def ctx (loom/start!)) (println \"Loom nREPL server started on port 7888\") @(promise)" &
```

Wait a few seconds for the server to start.

## Step 3 — Verify Loom is running
